...
_03_03_02_kernel_cfg_fstab() {
    ...
    then
        ...
        printf "UUID=$(blkid -o value -s UUID $PART_EFI)\t/boot/efi\tvfat\tdefaults,noatime\t0 1\n" | tee -a /mnt/etc/fstab
    fi
...
_03_06_setup_boot_env() {
    ...
    KERNEL_CMDLINE="cryptdevice=UUID=$(cryptsetup luksUUID $PART_LUKS):$LUKS_LVM root=/dev/$LVM_VG/$LV_ROOT rw vfio-pci.disable_idle_d3=1"
